...
Ignore accents (called character folding in Emacs) in incremental searches.
...
Note that it only works in non-regexp searches. You can turn it off inside
an incremental search by typing =M-s '= and you can turn it on in the
same way inside a regexp search.

Emacs already does case folding by default. This can also be turned
off using =M-s c= in an incremental search.
...
** Winner mode
    ...
    we'll assign C-c-right for rotating windows. Ediff always messes
    up windows, so we add a hook to automatically undo window changes.
